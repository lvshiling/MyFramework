...
namespace RemoteDebugger {
    public class C2SHandlers {
        ...
        private bool Modify<T1, T2>(T1 obj, string szName, T2 value) {
            try {
        ...
        private bool C2S_SetObjectLayer(NetCmd cmd, Cmd c) {
            ...
            foreach (var transform in transformList)
                Modify<GameObject, int>(transform.gameObject, "layer", obj.m_nLayer);

                GameObj[] objs = new GameObj[transformList.Count];

                for (int i = 0; i < objs.Length; ++i) {
                    objs[i] = new GameObj(transformList[i].gameObject);
                }
        ...
        private bool C2S_EnableComponent(NetCmd cmd, Cmd c) {
            ...
            try {
                ...
                ComponentExtension.SetValue(comp, "enabled", rdComp.m_bEnable);
            }
            catch (Exception ex) {
