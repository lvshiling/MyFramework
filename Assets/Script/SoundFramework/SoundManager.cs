    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            foreach (int key in keys)
            {
                ...
                if ( !audio.playing && !audio.paused && !audio.audioSource.isPlaying )
                {
                    Destroy( audio.audioSource );
                    musicAudio.Remove( key );
                }
        ...
        public static int PlayMusic( string _name, float _volume = 1f, bool _loop = false, bool _persist = false, 
        float _fadeInSeconds = 1f, float _fadeOutSeconds = 1f, float _currentMusicfadeOutSeconds = -1f, Transform _source = null )
        {
            if( ignoreDuplicateMusic )
            {
                ...
                foreach( int key in keys )
                {
                    if( musicAudio[key].audioSource.clip.name == _name )
                    {
                        return musicAudio[key].audioID;
                    }
                }
            }
            Audio audio = new Audio( Audio.AudioType.Music, null, _loop, _persist, _volume, _fadeInSeconds, _fadeOutSeconds, _source );
            ResManager.Instance.LoadAsset( string.Format( "Assets/Res/Audio/Dynamic/Music/{0}", _name ), ( _data, _obj )=> 
            {
                StopAllMusic( _currentMusicfadeOutSeconds );
                audio.initClip = _obj as AudioClip;
                audio.Play();
        ...
        public static int PlayMusic( AudioClip clip, float volume = 1f, bool loop = false, bool persist = false, 
        float fadeInSeconds = 1f, float fadeOutSeconds = 1f, float currentMusicfadeOutSeconds = -1f, Transform sourceTransform = null )
        {
            if ( clip == null )
            {
        ...
        /// <returns>The ID of the created Audio object</returns>
        public static int PlaySound( AudioClip clip, float volume = 1f, bool loop = false, Transform sourceTransform = null )
        {
            if (clip == null)
            {
                Debug.LogError("Sound Manager: Audio clip is null, cannot play music", clip);
        ...
        public static int PlaySound( string _name, float volume = 1f, bool loop = false, Transform sourceTransform = null )
        {
            if( ignoreDuplicateSounds )
            {
                List<int> keys = new List<int>( soundsAudio.Keys );
                foreach( int key in keys )
                {
                    if( soundsAudio[key].audioSource.clip.name == _name )
                    {
                        return soundsAudio[key].audioID;
                    }
                }
            ...
            Audio audio = new Audio( Audio.AudioType.Sound, null, loop, false, volume, 0f, 0f, sourceTransform );
            ResManager.Instance.LoadAsset( string.Format( "Assets/Res/Audio/Dynamic/Sound/{0}", _name ), (_data, _obj) =>
            {
                audio.initClip = _obj as AudioClip;
                audio.Play();
                soundsAudio.Add( audio.audioID, audio );
        ...
        /// <param name="volume"> The volume the music will have</param>
        /// <returns>The ID of the created Audio object</returns>
        public static int PlayUISound( AudioClip clip, float volume = 1f )
        {
            if (clip == null)
            {
                Debug.LogError("Sound Manager: Audio clip is null, cannot play music", clip);
        ...
        public static int PlayUISound( string _name, float volume = 1f )
        {
            if( ignoreDuplicateUISounds )
            {
                List<int> keys = new List<int>( UISoundsAudio.Keys );
                foreach( int key in keys )
                {
                    if( UISoundsAudio[key].audioSource.clip.name == _name )
                    {
                        return UISoundsAudio[key].audioID;
                    }
                }
            }
            Audio audio = new Audio( Audio.AudioType.UISound, null, false, false, volume, 0f, 0f, null );
            ResManager.Instance.LoadAsset( string.Format( "Assets/Res/Audio/Dynamic/Sound/{0}", _name ), (_data, _obj) =>
            {
                audio.initClip = _obj as AudioClip;
                audio.Play();
